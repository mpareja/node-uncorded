...
module.exports = function createRetryJsonStream(url) {
  ...
  function stop() {
    ...
    splitStream.removeListener('connect', onConnect);
    splitStream.removeListener('connectError', onConnectionError);
  ...
  function onConnectionError() {
    // capture current request so we can remove listeners,
    // removing listeners during 'error' event was causing problems
    const r = req;
    setImmediate(() => r.removeAllListeners());
